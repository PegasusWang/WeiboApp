...
class Upload(object):
    ...
    def upload_ido(self, **kwargs):
        beg, end = 1, 1122
        for i in range(beg, end+1):
            time.sleep(3)
            url = 'http://idolmaniax.tumblr.com/page/%s' % i
            print url
            leancloud_upload = self._upload
            spider = IdolmaniaxSpider()
                ...
                if each_url:
                    ...
                    if leancloud_upload.is_img_file(filename) and \
                        not leancloud_upload.exist_file(filename):
                            ...
                            time.sleep(2)
dict_list = [
    dict(upload_type='ido', class_name='Idolmaniax'),
]
