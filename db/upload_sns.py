...
from ..crawler.girl.girls_tumblr import (
    ...
    KormodelsSpider, Girl2chickSpider, SnsdpicsSpider,
)
...
class Upload(object):
    def __init__(self, **kwargs):
        ...
        self.map_method = {
            ...
            'upload_snsdpics': self.upload_snsdpics,
        }
    ...
    def upload_snsdpics(self, **kwargs):
        beg, end = 1, 375
        for i in range(beg, end+1):
            time.sleep(2)
            url = 'http://snsdpics.com/page/%s' % i
            print url
            leancloud_upload = self._upload
            spider = SnsdpicsSpider()
            img_list = spider.get_img(url)
            for each_url in img_list:
                if each_url:
                    print each_url
...
dict_list = [
    ...
    #dict(upload_type='girl2chick', class_name='Girl2chick'),
    dict(upload_type='snsdpics', class_name='Snsdpics'),
]
