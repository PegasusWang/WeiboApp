...
from ..crawler.girl.girls_tumblr import (
    ...
    KormodelsSpider, Girl2chickSpider,
)
...
class Upload(object):
    def __init__(self, **kwargs):
        ...
        self.map_method = {
            ...
            'upload_girl2chick': self.upload_girl2chick,
        }
    ...
    def upload_girl2chick(self, **kwargs):
        beg, end = 1, 700
        for i in range(beg, end+1):
            time.sleep(2)
            url = 'http://girl2chick.tumblr.com/page/%s' % i
            print url
            leancloud_upload = self._upload
            spider = Girl2chickSpider()
            img_list = spider.get_img(url)
            for each_url in img_list:
                if each_url:
                    print each_url
