...
class Upload(object):
    ...
    def upload_sexy(self, **kwargs):
        beg, end = 1, 350
        for i in range(beg, end+1):
            time.sleep(2)
            url = 'http://sexy-lady-japan.tumblr.com/page/%s' % i
            print url
            leancloud_upload = self._upload
            spider = SexyLadyJapanSpider()
                ...
                if each_url:
                    ...
                    if leancloud_upload.is_img_file(filename) and \
                        not leancloud_upload.exist_file(filename):
                            ...
                            time.sleep(3)
dict_list = [
    dict(upload_type='sexy', class_name='SexyLadyJapan'),
]
