...
class Upload(object):
    def __init__(self, **kwargs):
        ...
        self.map_method = {
            'upload_small': self.upload_small,
        }
    ...
    def upload_small(self, **kwargs):
        beg, end = 1, 1900
        for i in range(beg, end+1):
            time.sleep(3)
            url = 'http://small-pig.tumblr.com/page/%s' % i
            print url
            leancloud_upload = self._upload
            spider = SmallPigSpider()
                ...
                if each_url:
                    ...
                    if leancloud_upload.is_img_file(filename) and \
                        not leancloud_upload.exist_file(filename):
                            ...
                            time.sleep(3)
dict_list = [
    dict(upload_type='small', class_name='SmallPig'),
]
