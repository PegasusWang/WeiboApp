...
class Upload(object):
    ...
    def upload_kf(self, **kwargs):
        beg, end = 1, 300
        for i in range(beg, end+1):
            time.sleep(2)
            url = 'http://korean-fashion.tumblr.com/page/%s' % i
            print url
            leancloud_upload = self._upload
            spider = KoreanFashionSpider()
                ...
                if each_url:
                    ...
                    if leancloud_upload.is_img_file(filename) and \
                        not leancloud_upload.exist_file(filename):
                            ...
                            time.sleep(2)
dict_list = [
    dict(upload_type='kf', class_name='KoreanFashion'),
]
