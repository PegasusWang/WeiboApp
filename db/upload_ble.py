...
class Upload(object):
    ...
    def upload_ble(self, **kwargs):
        beg, end = 1, 6000
        for i in range(beg, end+1):
            time.sleep(3)
            url = 'http://blendy99.tumblr.com/page/%s' % i
            print url
            leancloud_upload = self._upload
            spider = Blendy99Spider()
                ...
                if each_url:
                    ...
                    if leancloud_upload.is_img_file(filename) and \
                        not leancloud_upload.exist_file(filename):
                            ...
                            time.sleep(3)
dict_list = [
    dict(upload_type='ble', class_name='Blendy99'),
]
