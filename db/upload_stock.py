...
class Upload(object):
    def __init__(self, **kwargs):
        ...
        self.map_method = {
            'upload_stock': self.upload_stock,
        }
    ...
    def upload_stock(self, **kwargs):
        beg, end = 1, 900
        for i in range(beg, end+1):
            time.sleep(3)
            url = 'http://sexy-in-stockings.tumblr.com/page/%s' % i
            print url
            leancloud_upload = self._upload
            spider = SexyInStockingsSpider()
...
dict_list = [
    dict(upload_type='stock', class_name='SexyInStockings'),
]
