...
class Upload(object):
    def __init__(self, **kwargs):
        ...
        self.map_method = {
            'upload_dogsand': self.upload_dogsand,
        }
    ...
    def upload_dogsand(self, **kwargs):
        beg, end = 1, 1000
        for i in range(beg, end+1):
            time.sleep(3)
            url = 'http://dogsandpupsdaily.tumblr.com/page/%s' % i
            print url
            leancloud_upload = self._upload
            spider = DogsandpupsdailySpider()
...
dict_list = [
    dict(upload_type='dogsand', class_name='Dog'),
]
