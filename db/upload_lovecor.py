...
class Upload(object):
    def __init__(self, **kwargs):
        ...
        self.map_method = {
            'upload_lovecor': self.upload_lovecor,
        }
    ...
    def upload_lovecor(self, **kwargs):
        beg, end = 1, 1000
        for i in range(beg, end+1):
            time.sleep(3)
            url = 'http://lovecorgis.tumblr.com/page/%s' % i
            print url
            leancloud_upload = self._upload
            spider = LovecorgisSpider()
...
dict_list = [
    dict(upload_type='lovecor', class_name='Dog'),
]
