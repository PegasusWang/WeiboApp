...
class Upload(object):
    ...
    def upload_dou(self, **kwargs):
        beg, end = 1, 800
        for i in range(beg, end+1):
            time.sleep(3)
            url = 'http://www.doumigirls.com/page/%s' % i
            print url
            leancloud_upload = self._upload
            spider = DoumigirlsSpider()
                ...
                if each_url:
                    ...
                    if leancloud_upload.is_img_file(filename) and \
                        not leancloud_upload.exist_file(filename):
                            ...
                            time.sleep(3)
dict_list = [
    dict(upload_type='dou', class_name='Doumigirls'),
]
